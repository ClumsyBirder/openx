...
export function Header(): React.JSX.Element {
  ...
  return (
    <header className="drag-region min-h-10 flex items-center select-none py-1">
      <div className="no-drag relative ml-2 flex items-center" ref={dropdownRef}>
        <button
          type="button"
        ...
        >
          ...
          <span className="min-w-0 flex flex-col gap-0.5" title={headerTitle}>
            <span className="text-sm font-medium leading-tight truncate">{headerPrimary}</span>
            {headerSecondary ? (
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className="w-8 h-8 flex items-center justify-center hover:bg-accent transition-colors rounded-lg mx-3"
            >
              {installing ? (
